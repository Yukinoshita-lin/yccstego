...
- `-t/--truncate`：消息超容量时按 UTF-8 安全边界截断；默认超容量则报错退出。
  （注意：仅对“正文载体不足”生效；若图像小到连头部载体都放不下，`--truncate` 无效，仍会报“图像太小”。）
...
- **提示“正文载体不足”**（消息太长或 `-p` 过大）：加 `--truncate` 自动按 UTF-8 安全截断，或换更大的图、调高 `-q`、减小 `-p`。
- **提示“图像太小”**（可用的非零 AC 载体连头部都放不下）：此时 `--truncate` **无效**（头部不足发生在截断判断之前），只能换更大的图。
